...
public class UberProgressView extends View {
    ...
    private float rOrbiting; // 轨道圆的半径
    ...
    private final Paint mPaintOrbitingCircle1 = new Paint(); // 这个轨道上的四个圆
    ...
    private void setupInitialValuesForAnimation() {
        orbitPathDistanceFromCenter = 4 * rStationary; // 中心圆的半径乘以4，得到轨迹圆的半径
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawCircle(canvas, theta, mPaintOrbitingCircle1); // TODO!!!这个是画轨道圆的
    ...
    private void drawCircle(Canvas canvas, float theta, Paint paint) {
        ...
        double thetaInRadians = Math.toRadians(theta); //TODO!!!注意这个函数
        float oribitingCX, oribitingCY;

        if (roationDirection == 0) {
            // 公式还是很简单的，根据外轨道圆的半径得出相应的轨迹圆的圆心坐标
